...
def score_summary(exp_id=None):
    ...
    with get_conn() as conn:
        where = "WHERE exp_id = ?" if exp_id else ""
        params = (exp_id,) if exp_id else ()
        rows = conn.execute(f"""
            SELECT tier, modelo_generador,
                   ROUND(AVG(score_compuesto), 3) AS mean_score,
                   ROUND(AVG(M1), 3) AS mean_M1,
                   ROUND(AVG(M3), 3) AS mean_M3,
                   ROUND(AVG(M5), 3) AS mean_M5,
                   COUNT(*) AS n
    ...
    with get_conn() as conn:
        rows = conn.execute("""
            SELECT e.exp_id, e.started_at, lv.tier,
                   ROUND(AVG(lv.score_compuesto), 3) AS mean_score,
                   COUNT(*) AS n
            FROM llm_evaluations lv
            JOIN experiments e ON lv.exp_id = e.exp_id
            GROUP BY e.exp_id, lv.tier
...
def human_vs_llm_agreement(exp_id):
    ...
    with get_conn() as conn:
        llm_rows = conn.execute("""
            SELECT tier, modelo_generador,
                   ROUND(AVG(M1), 3) AS M1, ROUND(AVG(M3), 3) AS M3,
                   ROUND(AVG(M5), 3) AS M5, COUNT(*) AS n
            FROM llm_evaluations WHERE exp_id = ?
            GROUP BY tier, modelo_generador
        """, (exp_id,)).fetchall()
        ...
        human_rows = conn.execute("""
            SELECT tier, modelo_generador,
                   ROUND(AVG(M1), 3) AS M1, ROUND(AVG(M3), 3) AS M3,
                   ROUND(AVG(M5), 3) AS M5, COUNT(*) AS n
            FROM human_evaluations WHERE exp_id = ?
            GROUP BY tier, modelo_generador
        """, (exp_id,)).fetchall()
...
def get_first_unsubmitted(exp_id, evaluator, all_species):
    ...
    with get_conn() as conn:
        rows = conn.execute("""
            ...
            WHERE exp_id=? AND evaluator=?
        """, (exp_id, evaluator)).fetchall()
    done = {}
    for r in rows:
...
def upsert_expert_session(username, model_A, model_B):
    ...
    with get_conn() as conn:
        existing = conn.execute(
            "SELECT * FROM expert_sessions WHERE username = ?", (username,)
        ).fetchone()
        if existing:
            return dict(existing)
        conn.execute(
            "INSERT INTO expert_sessions (username, model_A, model_B, created_at) VALUES (?,?,?,?)",
